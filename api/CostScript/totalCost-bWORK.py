...
web3 = Web3(Web3.HTTPProvider('https://base-mainnet.g.alchemy.com/v2/G9pwTaVoKOkRTfL_0Ub-8BxoKp0DspwZ'))

TOKEN_ADDRESS = '0x677F124CD209489F6621eebDFdA1E32c625214d4'
MINT_ADDRESS = '0x2f38B1a516239739CdCD2C228D1Eb96E29800975'
...
def docrap(log):
    ...
    if from_address == '0x2f38b1a516239739cdcd2c228d1eb96e29800975' and value != 0:
        tx_hash = log['transactionHash']
        ...
        cost, who_sent = get_transaction_cost(tx_hash)
        return {
            'tx_hash': tx_hash.hex(),
            'from': who_sent,
            'to': to_address,
...
def save_results(address_totals, filename_prefix='BWORK_transaction_analysis'):
    ...
    for txn in combined_data:
        ...
        if from_address not in aggregated_data:
            # Initialize entry if not already present
            aggregated_data[from_address] = {
                "from": from_address,
                "total_value": 0,
                "total_cost": 0,
                "transaction_count": 0
        ...
        aggregated_data[from_address]["total_value"] += txn["value"]
        aggregated_data[from_address]["total_cost"] += txn["cost"]
        aggregated_data[from_address]["transaction_count"] +=1
...
if __name__ == '__main__':
    ...
    start_block = last_block #25990908 #last_block
    #end_block =  last_block  + 100 
